...
class CMilter {
    ...
    _read_response (cb) {
        ...
        const read_timeout = this.config.timeouts.command || 1000
...
class MilterStream extends stream.Duplex { // milter socket decorator
    ...
    _write_multi_command ({cmd, cmd_char, data, expect_response}, enc, done) {
        ...
        const create_mutex = () => new Promise(resolve => this._consume_it_myself = resolve)
        ...
        chunker
            ...
            .on('close', (had_error) => {
                dbgmlog(`_write_multi_command: all done. ${had_error ? 'Completed with ERROR state.' : ''}`)
                // chunker.off('snubbed', unsnubber)
                done_once()
            })
